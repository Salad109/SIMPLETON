...
ax.set_title('Total Processing Time vs Knot Gap (s)', fontsize=14, fontweight='bold')
...
ax.set_title('Time Breakdown by Knot Gap (s)', fontsize=14, fontweight='bold')
...
ax.set_xlabel(param_label, fontsize=12)
ax.set_ylabel('Time (s)', fontsize=12)
ax.set_title('Time Breakdown Stacked by Knot Gap (s)', fontsize=14, fontweight='bold')
...
# 5 - miss distance error on matched events
fig, ax = plt.subplots(figsize=(10, 6))
floor = 1e-3
ax.plot(avg[param], avg['miss_err_p99_m'].clip(lower=floor), 'o-', color='#2E86AB',
        linewidth=2, markersize=7, label='p99')
ax.plot(avg[param], avg['miss_err_median_m'].clip(lower=floor), 's--', color='#d4a34a',
        linewidth=2, markersize=6, label='median')
ax.set_yscale('log')
ax.set_xlabel(param_label, fontsize=12)
ax.set_ylabel('Miss distance error vs baseline (m, floored at 1 mm)', fontsize=12)
ax.set_title('Miss Distance Error on Matched Events', fontsize=14, fontweight='bold')
ax.legend(fontsize=11)
ax.grid(True, alpha=0.3, which='both')
plt.tight_layout()
plt.savefig('5_miss_error.png', dpi=300, bbox_inches='tight')
plt.close()
